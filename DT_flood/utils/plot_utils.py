...
def plot_wflow_model(wflow_model):
    ...
    if "reservoirs" in wflow_model.geoms:
        resv_kwargs = dict(facecolor='blue', edgecolor='black', linewidth=1, label='reservoirs')
        wflow_model.staticgeoms['reservoirs'].plot(ax=ax,**resv_kwargs)
    if 'Q_gauges' in wflow_model.results:
        gauges_kwargs = dict(facecolor='red', marker='d', markersize=30, zorder=10, edgecolor='w', label='gauges')
        gdf_gauges = gdf.GeoDataFrame({'geometry': wflow_model.results['Q_gauges'].geometry})
...
def plot_fiat_model(fiat):
    ...
    for gdf in by_type:
        ...
        handles.append(gdf.plot(ax=ax, column="Max Potential Damage: Total", scheme='quantiles', cmap=get_cmap(cmaps[class_name])))
