...
def create_scenario_config(database: IDatabase, scenario_config: dict) -> IScenario:
    ...
    scenarios.create_scenario(attrs=scenario_dict, database=database)
...
def create_strategy(database: IDatabase, scenario_config: dict) -> IStrategy:
    ...
    if not scenario_config["strategy"]["name"] in strategies_existing['name']:
        strategy_new = create_scenario_config(database, scenario_config)
        strategies.save_strategy(strategy_new, database)
        return strategy_new
    else: return strategies.get_strategy(scenario_config["strategy"]["name"])

def create_scenario_config(database: IDatabase, scenario_config: dict) -> IStrategy:
